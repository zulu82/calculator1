...
func isRomanNumber(s string) bool {
	return regexp.MustCompile(`^[IVXLCDM]+$`).MatchString(s) // Исправлен регулярное выражение для корректной проверки
}

// Проверка на допустимые римские цифры
func isValidRoman(s string) bool {
	// Регулярное выражение для проверки правильности римских чисел.
